...
export const ProposeMergeDecision = Effect.fn(
  ...
  const { reviewRun, findings } = yield* withCriticalPathTransition(
    {
      ...transitionContext,
      operation: 'proposeMergeDecision.review',
      stage: criticalPathStages.review,
    },
    Effect.gen(function* () {
      const startedAt = yield* Clock.currentTimeMillis
      const review = yield* reviewer.runReview(input)
      const completedAt = yield* Clock.currentTimeMillis
      const persistedReviewRun = yield* storage.recordReviewRun({
        workflowRunId: input.workflowRunId,
      ...
      const persistedFindings = yield* Effect.forEach(
        review.findings.map((finding, index) => ({ finding, index })),
        ({ finding, index }) =>
          storage.recordReviewFinding({
            workflowRunId: input.workflowRunId,
            reviewRunId: persistedReviewRun.id,
            ...
            idempotencyKey: `${persistedReviewRun.id}:finding:${index}`,
  ...
  const policyDecision = yield* withCriticalPathTransition(
    {
      ...transitionContext,
      operation: 'proposeMergeDecision.policy',
    ...
    Effect.gen(function* () {
      ...
      const policyInputDigest = yield* sha256Json({
        ...
        verificationResultIds: sortedStrings(
          verificationCoverage.consideredResultIds,
        ),
        missingRequirementIds: sortedStrings(
          verificationCoverage.missingRequirementIds,
        ),
        policyVersion,
      })
      return yield* storage.recordPolicyDecision({
        workflowRunId: input.workflowRunId,
        reviewRunId: reviewRun.id,
        ...(input.candidatePatchSet === undefined
          ? {}
