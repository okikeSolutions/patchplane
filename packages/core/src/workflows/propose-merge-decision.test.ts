  ...
  it.effect('records review findings and a conservative policy decision', () =>
    Effect.gen(function* () {
      ...
      const storageLayer = Layer.succeed(StorageService, StorageService.of({
        ...
        recordCandidatePatchSet: (input) => Effect.succeed({ id: makeCandidatePatchSetId('patch-set-1'), ...input, createdAt: input.createdAt ?? 1 }),
        ...
        recordReviewRun: (input) =>
          Effect.suspend(() => {
            ...
            return Effect.succeed({ id: makeReviewRunId('review-run-1'), ...input, createdAt: input.createdAt ?? 1 })
          }),
        recordReviewFinding: (input) =>
          Effect.suspend(() => {
            recorded.push({ type: 'finding', value: input })
            return Effect.succeed({ id: makeReviewFindingId(`finding-${recorded.length}`), ...input, createdAt: input.createdAt ?? 1 })
          }),
        recordPolicyDecision: (input) =>
          Effect.suspend(() => {
            recorded.push({ type: 'policy', value: input })
            return Effect.succeed({ id: makePolicyDecisionId('policy-1'), ...input, createdAt: input.createdAt ?? 1 })
          }),
        recordPublicationResult: (input) => Effect.succeed({ id: makePublicationResultId('publication-1'), ...input, createdAt: input.createdAt ?? 1 }),
        recordProvenanceEvent: (input) => Effect.succeed({
      ...
      const result = yield* ProposeMergeDecision({
        ...
        verificationResults: [{
          ...
          workflowRunId,
          requirementId: makeVerificationRequirementId('verification-requirement-1'),
      ...
      }).pipe(Effect.provide(Layer.mergeAll(storageLayer, AlphaReviewServiceLayer, AlphaPolicyServiceLayer, cryptoLayer)))
