...
describe('Daytona sandbox boundary adapters', () => {
  it.effect('maps stable Daytona create defaults', () =>
    Effect.sync(() => {
      const params = toDaytonaCreateSandboxParams(config(), {
        traceId: 'trace-1',
  ...
  it.effect(
    'normalizes Daytona config into PatchPlane sandbox policy metadata',
    () =>
      Effect.sync(() => {
  ...
  it.effect(
    'deletes sandbox and disposes the client when startup fails',
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const exit = yield* service
          .runRepositoryCommand(commandInput)
          .pipe(Effect.exit)
  ...
  it.effect(
    'deletes an automatically retained RPC sandbox when setup fails',
    () => {
      const sandbox = fakeSandbox({
        git: {
          clone: vi.fn(async () => {
  ...
  it.effect(
    'preserves an RPC sandbox on failure when inspection retention is explicit',
    () => {
      const sandbox = fakeSandbox({
        git: {
          clone: vi.fn(async () => {
  ...
  it.effect(
    'deletes sandbox and command session when command execution throws',
    () => {
      const sandbox = fakeSandbox({
        process: {
          createSession: vi.fn(async () => undefined),
  ...
  it.effect(
    'collects diff, test report, and screenshot evidence before deleting the sandbox',
    () => {
      const executeSessionCommand = vi.fn(
        async (_sessionId: string, request: { readonly command: string }) => {
          if (request.command.includes('git rev-parse HEAD')) {
          ...
          if (
            request.command.includes('PATCHPLANE_ARTIFACT_BODY') &&
            request.command.includes('test-report.json')
          ) {
            return {
              exitCode: 0,
              stdout:
        ...
        expect(executeSessionCommand).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            command: expect.stringContaining(
              `git diff --binary --no-ext-diff '${repositoryBaseSha}' -- .`,
            ),
          }),
          expect.any(Number),
        )
  ...
  it.effect('fails closed when stale test-report cleanup fails', () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(
        result.evidenceArtifacts?.some(
          (artifact) => artifact.kind === 'test-report',
        ),
      ).toBe(false)
  ...
  it.effect(
    'runs Pi in JSON mode and returns normalized runtime events',
    () => {
      const stdout = [
  ...
  it.effect(
    'retains sandbox instead of deleting when retain mode is enabled',
    () => {
      const sandbox = fakeSandbox()
      const client = fakeClient(sandbox)
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const sandbox = {
          process: { createSession, executeSessionCommand, deleteSession },
        }
  ...
  it.effect('parses Pi JSON output into normalized runtime events', () =>
    Effect.gen(function* () {
      const output = [
        ...
        JSON.stringify({
          type: 'message_end',
          message: { content: [{ text: 'All checks passed.' }] },
        }),
        'not-json',
      ].join('\n')
      const parsed = yield* parsePiJsonRuntimeEventsEffect(output, {
        now: () => 123,
      })
  ...
  it.effect(
    'treats missing runtime session during hard terminate as already terminated',
    () => {
      const sandbox = fakeSandbox({
        process: {
          createSession: vi.fn(async () => undefined),
  ...
  it.effect('wraps Daytona streaming log callbacks as an Effect stream', () =>
    Effect.gen(function* () {
      ...
      const chunks = yield* streamSandboxSessionCommandLogs(
        sandbox,
        'session-1',
        'cmd-1',
  ...
  it.effect('starts an async Daytona session command handle', () =>
    Effect.gen(function* () {
      ...
      function getSessionCommandLogs(
        sessionId: string,
        commandId: string,
      ): Promise<{ readonly stdout: string; readonly stderr: string }>
      function getSessionCommandLogs(
  ...
  it.effect(
    'fails async Daytona session startup when command id is missing',
    () =>
      Effect.gen(function* () {
        const sandbox = {
          process: {
  ...
  it.effect(
    'maps provider API keys from Effect Config to Pi environment variables',
    () =>
      Effect.gen(function* () {
        const anthropic = yield* piRuntimeEnvironment({
          provider: 'anthropic',
        }).pipe(
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({ env: { ANTHROPIC_API_KEY: 'key' } }),
            ),
          ),
        )
        expect(anthropic).toEqual({ ANTHROPIC_API_KEY: 'key' })

        const copilot = yield* piRuntimeEnvironment({
          provider: 'github-copilot',
        }).pipe(
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({ env: { COPILOT_GITHUB_TOKEN: 'key' } }),
            ),
          ),
        )
        expect(copilot).toEqual({ COPILOT_GITHUB_TOKEN: 'key' })

        const cloudflare = yield* piRuntimeEnvironment({
        ...
        const missingOpenAi = yield* piRuntimeEnvironment({
          provider: 'openai',
        }).pipe(
          Effect.provide(
