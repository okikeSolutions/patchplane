...
const publishDecisionToSourceControl = Effect.fnUntraced(function*(input: {
  ...
  return yield* Effect.tryPromise({
      ...
      return await patchPlaneRuntime.runPromise(
        client.execute(
          HttpClientRequest.post('https://source-control-worker/internal/decision/publish', {
            ...
            body: HttpBody.text(JSON.stringify(decisionPublicationRequest(input)), 'application/json'),
          }),
        ).pipe(
    ...
    catch: () => new ReviewDecisionError({ message: 'Decision publication request failed' }),
  })
})
...
export const submitReviewDecisionServerFn = effectServerFn({
  ...
  effect: (input, context) => Effect.gen(function*() {
    ...
    if (comment.length === 0) {
      return yield* new ReviewDecisionError({ message: 'Decision comment required' })
    }
    ...
    const decision = yield* Effect.tryPromise({
      try: () => convex.mutation(recordHumanDecisionMutation, {
        workflowRunId: input.workflowRunId,
        ...
        ...(input.verificationOverrideReason === undefined
          ? {}
          : { verificationOverrideReason: input.verificationOverrideReason.trim() }),
        idempotencyKey: input.idempotencyKey,
      }),
      catch: () => new ReviewDecisionError({ message: 'Unable to record the review decision' }),
    }).pipe(Effect.flatMap(Schema.decodeUnknownEffect(RecordedHumanDecision)))
    ...
    if (!response.ok) {
      return yield* new ReviewDecisionError({ message: response.error ?? 'Decision publication failed' })
    }
