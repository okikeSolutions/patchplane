...
function composite(foreground: string, background: string, opacity: number) {
  ...
  const channels = left.map((value, index) =>
    Math.round((value * opacity + right[index] * (1 - opacity)) * 255),
  )
...
describe('app theme contrast contract', () => {
  test('keeps production dark surfaces and secondary text readable', () => {
    ...
    expect(
      contrast(token(darkTokens, 'muted-foreground'), card),
  ...
  test('keeps production light controls and tinted status text readable', () => {
    ...
    expect(
      contrast(token(rootTokens, 'brand-readable'), canvas),
    ).toBeGreaterThanOrEqual(4.5)
    expect(
      contrast(token(rootTokens, 'destructive-readable'), canvas),
    ).toBeGreaterThanOrEqual(4.5)
    expect(
