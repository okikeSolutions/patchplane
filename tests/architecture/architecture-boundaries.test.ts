...
describe('architecture boundaries', () => {
  ...
  it.effect('keeps hosted GitHub install and webhook routes wired across client and source-control Workers', () =>
    Effect.gen(function* () {
      ...
      expect(sourceControlWorker).toContain('syncGitHubInstallation(request, runtime)')
      expect(sourceControlWorker).toContain("url.pathname === '/api/github/webhook'")
      expect(sourceControlWorker).toContain('handleGitHubWebhook(request, env, runtime)')
      expect(sourceControlWorker).toContain("url.pathname === '/internal/runtime/control'")
      expect(sourceControlWorker).toContain('controlRuntimeSession(request, runtime)')
      ...
      expect(sourceControlGitHubRoutes).not.toContain('assertInternalAuthorization(request)')
      ...
      expect(clientWebhook).toContain('GitHub webhooks are handled by the dedicated GitHubWebhookWorker')
