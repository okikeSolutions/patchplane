...
Updated on 2026-07-29 from the current implementation and
[`acceptance-tests.md`](./acceptance-tests.md).

| Workstream            | Available foundation                                                                                                      | Active or implemented, evidence pending                                                                                                                                                                                                                         | Missing or blocked                                                                                                                                                   | Next gate                                                             |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| Candidate identity    | Immutable attempts, webhook-authenticated base/head pinning, exact bounded comparison capture, R2/Convex candidate freeze | Incoming-PR capture and dispatch fencing are implemented and hosted PR 150 proved the exact frozen candidate lineage                                                                                                                                           | Broader design-partner candidate coverage                                                                                                                               | Complete aggregate report coverage and canonical publication         |
...
| Execution             | Daytona lifecycle, exact checkout, Linux command execution, exact pre/post HEAD, and final-state candidate digests         | Hosted PR 150 proved endpoint-observed candidate-bound Linux execution with effective environment readback, durable asynchronous provider session/command identity, bounded terminal/log capture, and delete-to-not-found cleanup                              | Provider-attested protected source execution against transient edit/restore, exact provider-process resume, durable crash cleanup reconciliation, effective class/limit enforcement, Windows, and Computer Use | Close `EXEC-002`, then add candidate-bound read-only review and broader provider coverage |
| Evidence and coverage | R2 artifacts, Convex metadata, hashes, requirement coverage, Patch Report V1                                              | Hosted PR 150 persisted coherent candidate/plan/group/environment/process/test-report/diff/stdout/stderr/result/cleanup evidence; repository artifacts remain untrusted                                                                                          | Aggregate plan projection, broader isolation enforcement, and trusted external-check adapter                                                                          | Complete `PLAN-008` and add canonical publication                    |
...
| CAND-007 | P0       | Persisted candidate before verifier dispatch   | Candidate-bound freeze and dispatch leases require the durable incoming candidate, its Convex evidence row, and R2 metadata with a provider-validated matching checksum immediately before issuing the incoming dispatch claim; Daytona start/result writes are token-, lineage-, and sandbox-ID-fenced with timeout recovery | The exact incoming candidate record and artifact are observed before the incoming dispatch claim, and every result references that persisted candidate ID/digest; Convex and R2 do not provide continuous cross-store existence through provider start | Verified    | Dispatch-path missing-object and metadata-mismatch tests, R2 provider-checksum tests, fencing/recovery tests, and hosted PR 150 candidate lineage | —                               |
...
| EXEC-002 | P0       | Candidate mutation detection                 | Trusted Linux capture now requires exact HEAD before/after and matching frozen final-state digests, so persistent mutation or HEAD drift fails closed. Writable candidate-controlled execution can still edit and restore files between endpoint observations. | Persistent mutation or HEAD drift becomes `invalidated`; provider-attested protected source execution proves transient edit/restore cannot influence credited verification | In progress | Automated exact-HEAD/final-state invalidation coverage plus an explicit endpoint-snapshot limitation regression | Protected read-only candidate execution and negative live smoke |
| EXEC-003 | P0       | Per-requirement orchestration                | `RunIncomingVerificationPlan` executes each supported command once in a separate Daytona sandbox with a forced deletion attempt; unsupported/provider/error outcomes persist independently; interruptions and crashes are fenced and fail closed; delayed queue exhaustion terminalizes receipts and materialized requirements, while exact provider-process resume and crash cleanup reconciliation remain open | Each plan requirement has a separately timed, cancellable, persisted execution result; shared execution groups are explicit                                                                                       | Verified    | Core/Daytona/Convex recovery tests plus hosted PR 150 group/result evidence                                                                                           | —                                                                      |
| EXEC-004 | P0       | Control-plane result envelope                | Branded results bind plan/group/requirement/candidate, recomputed command digest and timeout, provider/platform/architecture, timing/exit, mutation digests, artifacts, log capture, and cleanup outcome; malformed provider envelopes fail closed                                                                                                                                                                   | PatchPlane records requirement, candidate, plan, provider, command identity, platform/architecture, timing, exit, digests, artifacts, and cleanup independently of tenant output                                  | Verified    | Automated envelope/replay/fencing/projection tests plus hosted PR 150 result `rs7bhcpct7ce5var1j33yftvs18ben6q`                                                     | —                                                                      |
| EXEC-005 | P0       | First-class command stdout/stderr            | Every trusted command attempts deterministic bounded stdout and stderr R2 capture; IDs and captured/truncated/failed status are durable, and anything other than complete capture prevents `passed`                                                                                                                                                                                                                  | Every command result references bounded stdout/stderr artifacts or an explicit capture failure                                                                                                                    | Verified    | Automated log/R2 tests plus hosted PR 150 captured stdout/stderr artifact IDs                                                                                         | —                                                                      |
...
| EXEC-010 | P1       | Resolved execution-environment identity      | Trusted Linux groups persist the PatchPlane-declared class source plus refreshed provider image/snapshot identity, target, OS, architecture, state, public/link/volume posture, observed resources/network/lifecycle, and observation time; mismatches prevent `passed`                                                                                                                                              | Every result records the Daytona sandbox class, OS, architecture, trusted image/snapshot, resources, and network policy used for that execution                                                                   | Verified    | Automated environment/invariant tests plus hosted PR 150 effective environment row                                                                                   | —                                                                      |
...
| EXEC-013 | P0       | Effective Daytona isolation evidence         | Hosted PR 150 recorded candidate-bound public/link/volume posture, requested resource/network/lifecycle readback, and structured delete-to-not-found; provider-observed class/limit enforcement, tier exceptions, crash reconciliation, prior-state isolation, and representative egress remain unproven                                                                                                                 | Live smokes record resolved boundary/limits, reject public preview/link/volume/persistent-state reuse, verify representative egress, and poll deletion to not-found                                               | In progress | Automated environment/deletion tests plus hosted PR 150 partial effective-isolation evidence                                                                           | EXEC-006, EXEC-010, EXEC-011                                           |
...
| LIVE-001 | P0       | Real incoming-PR trust loop              | Hosted PR 150 proved exact incoming candidate capture, trusted plan, endpoint-observed verification, artifacts, cleanup, review, and policy; protected source execution, aggregate plan projection, authenticated human decision, and canonical GitHub publication remain open | One real PR is captured as exact `base...head`, planned, frozen, verified with protected source execution, reviewed, decided in authenticated UI, and published | In progress | PR 150 M9.75 endpoint-observed evidence; protected execution and M10 decision/publication rows remain missing | EXEC-002, PLAN-008, REV-002, REV-004, PUB-001 through PUB-003 |
...
- [ ] 6. Prove one fully supported Linux PR end to end. PR 150 proved the endpoint-observed Linux lifecycle, but protected source execution remains open under `EXEC-002`.
...
| 2026-07-28 | EXEC-010, EXEC-013                     | Added bounded effective Daytona environment identity, provider refresh plus independent Linux OS/architecture probe, fail-closed requested/effective lifecycle/network/resource matching, public/link/volume rejection, provider image identity, durable asynchronous session/command identity before terminal/log polling, and cleanup credit only after bounded structured not-found polling                       | Full gate passed before queue remediation; hosted PR 131 proved queue receipt/workflow binding only                                                                    | Partial live evidence: direct PR 129 lifecycle/readback; PR 131 had an immutable empty plan                                                                   | Run a fresh non-empty supported Linux candidate                                                                                                                                                    |
| 2026-07-29 | Sequence 6 / CAND-005/007 / EXEC-003/004/005/010 | Bounded Daytona provider promises, used one non-PTY stateless provider request for each read-only/setup evidence probe while retaining durable async identity for the trusted command, corrected the test-report path, and confirmed Workflows do not reset the Free-plan 50-request budget across steps                                           | `bun run verify`; 37 Daytona boundary tests; Effect v4 beta source/tests; no-PR local, Worker, service-binding, source-runtime, Workflow-limit, and split-budget experiments | Hosted PR 150 workflow `ms7exrs2kbe5ng7gsrtk3m7c698bf449` passed exact candidate, plan, group, environment, process, artifacts, result, cleanup, review/policy, and terminal queue receipt | Continue with read-only Pi review and `PLAN-008`; keep crash reconciliation, Windows, Computer Use, and broader isolation/scale evidence open                                                   |
