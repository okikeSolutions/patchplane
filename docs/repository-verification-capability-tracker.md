...
| Execution             | Daytona lifecycle, exact checkout, Linux command execution, before/after candidate digests                                | Incoming candidate capture is separate; the current sandbox agent path still combines Pi activity and verification collection             | Trusted per-requirement orchestration, provider capabilities, independent execution groups, first-class command logs | Implement the bounded trusted plan and per-requirement executor               |
