...
`ADR 0001` currently describes only a sandbox-generated candidate. The alpha
cannot claim incoming-PR verification until that governing identity decision is
amended or superseded and the implementation follows it. Until then, the
product must report incoming PR verification as unavailable rather than publish
“verification passed.”
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Candidate identity    | Immutable attempts, exact intake head pinning, sandbox-generated candidate digest                                     | Candidate `baseSha`/`headSha` fields and exact-head publication contracts exist                                                           | PR candidate schema, base SHA intake, exact `base...head` capture, durable freeze before verification            | Implement the alpha PR candidate identity and freeze contract                 |
| Verification plan     | Requirements persist before execution; requirement/result storage supports arrays; non-Linux requirements fail closed | One test and one browser requirement can be configured through deployment environment                                                     | Versioned plan, trusted source/digest, precedence, bounded requirement list, candidate self-weakening prevention | Specify and decode `VerificationPlanV1`                                       |
| Execution             | Daytona lifecycle, exact checkout, Linux command execution, before/after candidate digests                            | Single sandbox collection combines generation, verification, and candidate return                                                         | Per-requirement orchestration, provider capabilities, independent execution groups, first-class command logs     | Split candidate capture from verification execution                           |
| Evidence and coverage | R2 artifacts, Convex metadata, hashes, requirement coverage, Patch Report V1                                          | Repository-produced test reports are validated                                                                                            | Control-plane result envelopes and trusted external-check adapter                                                | Persist stdout/stderr and execution facts independently of repository reports |
...
| Publication           | Canonical decision publication, leases, fencing, replay tests, exact-head check contract                              | Hosted V1 exact-head publication is not proven                                                                                            | Runtime candidate `headSha`, GitHub App `checks: write`, removal of duplicate preliminary reports                | Publish one canonical incoming-PR report and replay it live                   |
...
| CAND-003 | P0       | Explicit alpha PR candidate subject            | Candidate records model the sandbox-generated path and cannot identify an incoming PR patch                                 | PatchPlane-owned schema identifies an incoming PR candidate by repository, PR, base SHA, head SHA, exact diff digest, and source event without provider objects crossing core | Not started | Current `CandidatePatchSet` cannot represent the alpha PR subject          | Domain design                           |
| CAND-004 | P0       | PR base and head identity                      | Intake preserves head SHA but not the PR base SHA needed for exact comparison identity                                      | Signed webhook intake persists both base and head SHA with repository and PR identity                                                                                         | Not started | `normalize-github-webhook-event.ts` currently maps only `headSha`          | CAND-003                                |
| CAND-005 | P0       | Exact incoming-PR candidate capture            | Current candidate is the diff Pi creates after cloning the PR head                                                          | PatchPlane captures and hashes exact `baseSha...headSha` bytes before review or verification and stores `headSha` on the candidate                                            | Not started | No incoming-candidate freeze workflow exists                               | CAND-003, CAND-004                      |
...
| CAND-007 | P0       | Persisted candidate before verifier dispatch   | Current Daytona call combines generation, verification, and candidate return; the incoming PR candidate is not frozen first | The exact incoming candidate record and artifact exist before Daytona or Pi starts, and every result references that persisted candidate ID/digest                            | Not started | Current combined call cannot establish the alpha PR ordering               | ADR identity update, CAND-005, EXEC-003 |
...
| PLAN-002 | P0       | Versioned `VerificationPlanV1`                   | Individual requirements exist, but there is no plan identity or digest                               | Domain schema records version, trusted source, source revision, ordered requirements, limits, and SHA-256 digest                              | Not started | `packages/domain/src/verification.ts` has requirement/result schemas only | Domain design                |
| PLAN-003 | P0       | Trusted plan source and precedence               | Requirements come from deployment environment variables                                              | Effective plan resolves from non-negotiable system policy plus authorized workspace or pinned base-repository policy with explicit precedence | Not started | No repository/workspace plan resolver                                     | PLAN-002                     |
...
| PLAN-005 | P0       | Bounded multiple requirements                    | Storage and report projection accept arrays; configuration exposes one test plus one browser command | A decoded plan can persist and execute a documented maximum number of independently identified requirements                                   | Not started | Requirement/result collections already exist                              | PLAN-002, EXEC-003           |
...
| PLAN-007 | P1       | Safe plan validation                             | Current environment strings have command/platform validation but no whole-plan budget                | Invalid IDs, duplicate IDs, unsupported kinds, excessive timeouts/counts/artifact budgets, and interpolation are rejected before execution    | Not started | No plan schema                                                            | PLAN-002                     |
| PLAN-008 | P1       | Plan coverage in Patch Report                    | Reports show requirements/results and gaps but no effective plan identity                            | Report shows plan source, source revision, digest, required count, and passed/failed/blocked/error/invalidated counts                         | Not started | Patch Report V1 already projects individual requirements/results          | PLAN-002                     |
...
| EXEC-003 | P0       | Per-requirement orchestration                | One test and one browser capture run inside the agent sandbox operation                                                                                                | Each plan requirement has a separately timed, cancellable, persisted execution result; shared execution groups are explicit                                                                                       | Not started | No plan executor workflow                                                                                                              | PLAN-005, CAND-007                       |
| EXEC-004 | P0       | Control-plane result envelope                | Normalized results exist, but repository-produced reports carry much of the evidence burden                                                                            | PatchPlane records requirement, candidate, plan, provider, command identity, platform/architecture, timing, exit, digests, artifacts, and cleanup independently of tenant output                                  | Not started | Existing `VerificationResult` is narrower than the required alpha execution envelope                                                   | EXEC-003                                 |
| EXEC-005 | P0       | First-class command stdout/stderr            | Raw Pi JSONL and evidence artifacts persist; verification command logs are not consistently durable artifacts                                                          | Every command result references bounded stdout/stderr artifacts or an explicit capture failure                                                                                                                    | Not started | R2 infrastructure exists                                                                                                               | EXEC-003, EXEC-004                       |
...
| EXEC-007 | P0       | Bounded execution-group identity             | Current persistence allows one sandbox execution path and cannot dispatch separate Linux, Windows, or Computer Use groups                                              | The trusted plan defines bounded stable execution-group IDs; each group has an idempotent claim, environment identity, lifecycle, results, and explicit shared-state disclosure                                   | Not started | ADR 0001 now defines orchestration and per-group claims; storage/workflow implementation is missing                                    | EXEC-003, EXEC-006, EXEC-010             |
...
| EXEC-013 | P0       | Effective Daytona isolation evidence         | PatchPlane stores requested lifecycle/resource/network policy but does not prove effective tier exceptions, forbidden sharing/ingress, limits, or absence after delete | Live smokes record resolved boundary/limits, reject public preview/link/volume/persistent-state reuse, verify representative egress, and poll deletion to not-found                                               | Not started | Daytona isolation/persistence docs; requested-policy tests and Linux lifecycle smoke only                                              | EXEC-006, EXEC-010, EXEC-011             |
| EXEC-014 | P0       | Bounded concurrency and capacity outcomes    | Daytona supports fleets and shared-sandbox sessions; PatchPlane has no bounded multi-group scheduler or normalized rate/capacity result                                | Enforce plan/global concurrency and timeout budgets, forbid runtime resize, disclose shared sessions, terminate background work, honor bounded retries, and classify exhausted capacity as provider error/blocked | Not started | Daytona scale docs; current single-path timeout/session tests only                                                                     | PLAN-005, PLAN-007, EXEC-003, EXEC-007   |
...
| EVID-004 | P1       | Repository artifacts remain untrusted inputs  | Test report and screenshot capture validates expected files and metadata                               | Malformed, missing, stale, oversized, or identity-mismatched artifacts fail closed without controlling the command result envelope | Implemented | Daytona and persistence tests; control-plane envelope remains missing | EXEC-004           |
...
| PUB-002  | P0       | Exact-head check publication             | Core requires candidate `headSha`; real sandbox-generated candidates generally do not have one                   | Incoming PR candidates publish/update a check only against their exact head SHA using a stable external ID                      | Implemented | Core/GitHub adapter tests; incoming runtime path missing | CAND-005, LIVE-001                             |
...
- [ ] 2. Complete `CAND-003`/`CAND-004`: base/head subject at intake.
- [ ] 3. Complete `CAND-005`/`CAND-007`: R2-backed candidate freeze before dispatch.
- [ ] 4. Complete `PLAN-002`/`PLAN-003`/`PLAN-005`: bounded trusted plan.
- [ ] 5. Complete `EXEC-003`/`EXEC-004`/`EXEC-005`/`EXEC-007`: execution groups and command envelopes.
...
```text
amend or supersede ADR 0001 for the incoming PR candidate
→ CAND-003/CAND-004/CAND-005/CAND-007 exact candidate freeze
→ PLAN-002/PLAN-003/PLAN-005 bounded trusted plan
→ EXEC-003/EXEC-004/EXEC-005 PatchPlane-owned execution evidence
→ PLAN-008 report coverage
