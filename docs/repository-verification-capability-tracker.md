...
| Execution             | Daytona lifecycle, exact checkout, Linux command execution, before/after candidate digests                                | Hosted PR 150 proved fresh candidate-bound Linux execution with effective environment readback, durable asynchronous provider session/command identity, bounded terminal/log capture, and delete-to-not-found cleanup                                         | Exact provider-process resume from persisted command identity and durable crash cleanup reconciliation, effective class/limit enforcement, Windows, and Computer Use | Add candidate-bound read-only review and broader provider coverage   |
...
| LIVE-001 | P0       | Real incoming-PR trust loop              | Hosted PR 150 proved exact incoming candidate capture, trusted plan, fresh verification, artifacts, cleanup, review, and policy; authenticated human decision and canonical GitHub publication remain open | One real PR is captured as exact `base...head`, planned, frozen, verified, reviewed, decided in authenticated UI, and published | In progress | PR 150 M9.75 live evidence; M10 decision/publication rows remain missing | REV-002, REV-004, PUB-001 through PUB-003       |
...
- [x] 6. Prove one fully supported Linux PR end to end.
