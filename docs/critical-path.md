...
|     7 | What independently passed or failed?     | `RunIncomingVerificationPlan`, `PersistSandboxVerificationEvidence`, `evaluateVerificationCoverage`               | Correlate requirement, candidate, command, platform, architecture, artifacts, and digest before/after. Missing, blocked, errored, persistently mutated, final-HEAD-drifted, stale, truncated, or mismatched evidence is incomplete or failed; transient edit/restore remains outside the current endpoint-snapshot guarantee.                                                                                                                                                                             |
