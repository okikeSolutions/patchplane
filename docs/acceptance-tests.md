...
| M9.75     | Every bounded plan requirement executes in a fresh candidate-bound group rather than the agent sandbox                                     | fresh-group executor tests plus credentialed provider run                                                               | Missing   |
| M9.75     | GitHub PR intake freezes the webhook-authenticated base/head candidate and exact diff before Daytona or Pi starts                          | automated domain/intake/core/provider/backend freeze-order evidence passes; credentialed PR dogfood remains outstanding | Missing   |
...
| M9.75     | One orchestration claim dispatches bounded, idempotent execution groups for every required supported environment                           | storage/core/provider tests plus multi-environment smoke                                                                | Missing   |
