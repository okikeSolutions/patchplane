...
| `FND-002` | Shared Effect-backed domain package for core status and event data | `packages/domain/src/index.ts` |
...
| -- | ---- | ------ | -------------- | --------- |
| `MVP-001` | Promote `packages/domain` into a real shared contracts package and move backend-local runtime and sandbox contracts into it | `Active` | `packages/domain/src/index.ts`, `packages/backend/src/runtime/contracts.ts`, `packages/backend/src/sandbox/contracts.ts` | Clients, Convex, adapters, and workers all depend on one shared contracts package, and backend-only contract duplication is removed |
| `MVP-002` | Expand the shared `Effect` contract model to cover `WorkflowRun`, `RuntimeSession`, `MergeDecision`, `GitHubInstallation`, `RepositoryConnection`, and `WebhookDelivery` | `Active` | `packages/domain/src/index.ts` currently covers `PromptRequest`, `RuntimeEvent`, and `ReviewRun` only | The shared package contains the full product model needed for orchestration, publication, and provenance |
| `MVP-003` | Define shared `Effect` service boundaries for GitHub, runtime, sandbox, policy, artifacts, and interrupts | `Active` | `packages/backend/src/runtime/contracts.ts`, `packages/backend/src/sandbox/contracts.ts`, `packages/backend/src/policy/evaluate.ts` | `GitHubInstallationBroker`, `GitHubPublisher`, `RuntimeAdapter`, `SandboxAdapter`, `PolicyEvaluator`, `ArtifactStore`, and `InterruptController` exist behind shared contracts |
| `MVP-004` | Align Convex schema and function boundaries to shared contracts instead of duplicating shapes locally | `Active` | `packages/backend/convex/schema.ts` and `packages/backend/convex/requests.ts` still redefine workflow status and prompt scope | Convex ingress and egress decode shared contracts at the boundary, and local shape duplication is removed |
| `MVP-005` | Complete repository authority with GitHub App installation and verified webhook wiring | `Active` | WorkOS auth exists in `packages/backend/convex/auth.ts`, but there is no GitHub App or webhook implementation yet | One repo can be connected through a GitHub App, webhook signatures are verified, and inbound GitHub events can create prompt requests |
| `MVP-006` | Add durable Convex state for repositories, webhook deliveries, workflow runs, runtime sessions, publication records, approvals, and pending input | `Active` | `packages/backend/convex/schema.ts` has `promptRequests`, `runtimeEvents`, and `reviewRuns` only | The control plane has durable storage for the full request-to-publication lifecycle |
...
| `MVP-010` | Implement GitHub feedback publication for comments, checks, and PR or draft PR updates | `Open` | No publisher implementation exists yet | PatchPlane can publish one end-to-end execution result back to GitHub |
